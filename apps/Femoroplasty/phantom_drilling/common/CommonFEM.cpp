...
std::ostream& jhmr::fem::CommonProgArgs::vout()
{
  if (!verbose)
  {
    if (!null_vout)
...
void jhmr::fem::SetupCommonProgArgs(ProgOpts* po, const CommonProgArgs& args)
{
...
jhmr::fem::CommonProgArgs jhmr::fem::GetCommonProgArgs(const ProgOpts& po)
{
  CommonProgArgs args;
...
void jhmr::fem::SetupMaskProgArgs(ProgOpts* po, const std::string& default_name)
{
...
std::string jhmr::fem::GetMaskProgArgs(const ProgOpts& po)
{
  return po.get("mask-2d").as_string();
}

jhmr::Pt3 jhmr::fem::GetFHMidPtWrtVol(const H5::CommonFG& h5)
{
...
std::string jhmr::fem::GetFragRegiNameUsingMaskStr(const std::string frag_regi_str, const std::string& mask_arg)
{
...
std::string jhmr::fem::GetMaskProjGroupName(const std::string mask_arg)
{
  jhmrASSERT(!mask_arg.empty());
...
jhmr::size_type jhmr::fem::MapPatientIDToIndex(const std::string& pat_id)
{
...
std::string jhmr::fem::GetPatientID(const H5::CommonFG& h5)
{
  return ReadStringH5("specimen-id", h5);
}

std::string jhmr::fem::GetSideString(const H5::CommonFG& h5)
{
  return ReadStringH5("side", h5);
}

bool jhmr::fem::IsLeftSide(const H5::CommonFG& h5)
{
...
std::string jhmr::fem::GetAnatLandmarksMapH5Path()
{
  return "anat-landmarks";
}

jhmr::LandMap3
jhmr::fem::GetAnatLandmarksMap(const H5::CommonFG& h5)
{
  return ReadLandmarksMapH5<Pt3>(h5.openGroup(GetAnatLandmarksMapH5Path()));
}

jhmr::FrameTransform jhmr::fem::GetAPPToVolStdOrigin(const H5::CommonFG& h5)
{
...
jhmr::FrameTransform jhmr::fem::GetAPPToVolFHOrigin(const H5::CommonFG& h5)
{
  const auto anat_lands = GetAnatLandmarksMap(h5);
...
std::array<jhmr::Pt3,2> jhmr::fem::GetBothFHWrtVol(const H5::CommonFG& h5)
{
  const auto anat_lands = GetAnatLandmarksMap(h5);
...
jhmr::Pt3 jhmr::fem::GetIpsilFHPtWrtVol(const H5::CommonFG& h5)
{
  const auto fhs = GetBothFHWrtVol(h5);
...
jhmr::fem::PelvisFragFemurLabels
jhmr::fem::GetPelvisFragFemurLabels(const H5::CommonFG& h5, const LabelVol* seg)
{
  const auto fhs = GetBothFHWrtVol(h5);
...
std::string jhmr::fem::GetOrigIntensVolH5Path()
{
  return "vol";
}

std::string jhmr::fem::GetPelvisFemursSegH5Path()
{
  return "vol-seg";
}

std::string jhmr::fem::GetGTCutsSegH5Path()
{
  return "gt-cuts-seg";
}

std::string jhmr::fem::GetGTCutsDefH5Path()
{
  return "gt-cuts-fit-plan";
}

std::string jhmr::fem::GetActualCutsSegH5Path(const size_type cut_idx)
{
  return fmt::sprintf("rand-plans/%02lu/seg", cut_idx);
}

std::string jhmr::fem::GetActualCutsDefH5Path(const size_type cut_idx)
{
  return fmt::sprintf("rand-plans/%02lu/def", cut_idx);
}

std::string jhmr::fem::GetKnownPubisIliumCutsSegH5Path(const size_type cut_idx)
{
  return fmt::sprintf("rand-plans/%02lu/seg-known-pu-il", cut_idx);
}

std::string jhmr::fem::GetProjDataPath(const size_type cut_idx,
...
jhmr::VolPtr jhmr::fem::GetOrigIntensVol(const H5::CommonFG& h5)
{
  return ReadITKImageH5<PixelScalar,3>(h5.openGroup(GetOrigIntensVolH5Path()));
}

jhmr::VolPtr jhmr::fem::GetOrigIntensVolAsLinAtt(const H5::CommonFG& h5)
{
...
jhmr::LabelVolPtr jhmr::fem::GetPelvisSeg(const H5::CommonFG& h5)
{
  return ReadITKImageH5<LabelScalar,3>(h5.openGroup(GetPelvisFemursSegH5Path()));
}

jhmr::LabelVolPtr jhmr::fem::GetGTCutsSeg(const H5::CommonFG& h5)
{
  return ReadITKImageH5<LabelScalar,3>(h5.openGroup(GetGTCutsSegH5Path()));
}

H5::Group jhmr::fem::GetRandPlansParentGroup(const H5::CommonFG& h5)
{
  return h5.openGroup("rand-plans");
}

H5::Group jhmr::fem::GetReposParentGroup(const H5::CommonFG& h5)
{
  return h5.openGroup("repos");
}

std::vector<H5::Group> jhmr::fem::GetNumberedGroups(const H5::CommonFG& h5)
{
...
std::vector<H5::Group>
jhmr::fem::GetSubsetOfOneNumberedGroup(const std::vector<H5::Group>& gs, const size_type idx)
{
  return { gs[idx] };
}

std::vector<jhmr::FrameTransform>
jhmr::fem::GetInitCamWrtVols(const H5::CommonFG& h5)
{
...
jhmr::fem::FragRegiParams jhmr::fem::MakeFragRegiParamsOldSim(const bool use_patch_sim)
{
  FragRegiParams p;
...
jhmr::fem::FragRegiParams jhmr::fem::MakeFragRegiParamsNewSim(const bool use_patch_sim)
{
  FragRegiParams p;
...
jhmr::fem::FragRegiParams jhmr::fem::MakeFragRegiParamsNewSimPelvisFid(const bool use_patch_sim)
{
  FragRegiParams p;
...
jhmr::fem::FragRegiParams jhmr::fem::MakeFragRegiParamsNewSimReg(const bool use_patch_sim)
{
...
jhmr::fem::FragRegiParams jhmr::fem::MakeFragRegiParamsNewSimPelvisFidReg(const bool use_patch_sim)
{
  using FoldNormPDF = FoldedNormalPDF<CoordScalar>;
  using NormPDF     = NormalDist1D<CoordScalar>;
...
jhmr::fem::FragRegiParams jhmr::fem::MakePelvisRegiParamsRegAPView(const bool use_patch_sim, const size_type pop_size)
{
  using FoldNormPDF             = FoldedNormalPDF<CoordScalar>;
  using RotMagAndTransMagPen    = Regi2D3DSE3MagPenaltyFn<FoldNormPDF>;
  using NormPDF                 = NormalDist1D<CoordScalar>;
...
jhmr::fem::FragRegiParams jhmr::fem::MakePelvisRegiParamsRegRotFromAPView(const bool use_patch_sim, const size_type pop_size)
{
  using FoldNormPDF             = FoldedNormalPDF<CoordScalar>;
  using RotMagAndTransMagPen    = Regi2D3DSE3MagPenaltyFn<FoldNormPDF>;
  using NormPDF                 = NormalDist1D<CoordScalar>;
...
jhmr::fem::FragRegiParams
jhmr::fem::GetFragRegiParamsUsingCmdLineArgs(const bool use_old_sim_params,
...
std::string jhmr::fem::GetFullProjPathH5(const size_type cut_idx, const size_type repo_idx,
                                          const std::string data_name)
{
  return fmt::sprintf("rand-plans/%02lu/repos/%02lu/projs/%s", cut_idx, repo_idx, data_name);
}

std::string jhmr::fem::GetFullRegiPathH5(const size_type cut_idx, const size_type repo_idx,
                                          const std::string data_name,
...
std::string jhmr::fem::GetFullPelvisRegiPathH5(const size_type cut_idx, const size_type repo_idx, const std::string data_name,
                                                const std::string frag_regi_str,
                                                const size_type init_idx, const bool use_known_rel_views,
...
std::string jhmr::fem::GetFullRegiDebugPathH5(const size_type cut_idx, const size_type repo_idx,
                                               const std::string data_name,
                                               const std::string frag_regi_str,
                                               const size_type init_idx, const bool use_known_rel_views)
{
...
std::string jhmr::fem::GetFullPelvisRegiDebugPathH5(const size_type cut_idx, const size_type repo_idx, const std::string data_name,
                                                     const std::string frag_regi_str,
                                                     const size_type init_idx, const bool use_known_rel_views,
                                                     const int rel_view_idx)
{
...
void jhmr::fem::SetupMultiObjMultiLevelFragRegi(MultiLevelMultiObjRegi* ml_mo_regi, GPUPrefsXML* gpu_prefs,
...
void jhmr::fem::SetupPelvisMultiLevelRegi(MultiLevelMultiObjRegi* ml_mo_regi, GPUPrefsXML* gpu_prefs,
                                           const FragRegiParams& params, std::ostream& vout)
{
  using UseOtherVolCurEstForInit = MultiLevelMultiObjRegi::Level::SingleRegi::InitPosePrevPoseEst;
...
void jhmr::fem::SetupPelvisMultiLevelRegiSingleAPView(MultiLevelMultiObjRegi* ml_mo_regi,
...
void jhmr::fem::SetupPelvisMultiLevelRegiSingleRotFromAPView(MultiLevelMultiObjRegi* ml_mo_regi,
                                                              GPUPrefsXML* gpu_prefs,
                                                              const FragRegiParams& params,
...
cv::Mat jhmr::fem::OverlayLabelMapAsRGBOnGrayscale(const cv::Mat& gray_img,
...
void jhmr::fem::UpdateCountdownH5(Countdown* time_limit, H5::H5File& h5, std::ostream& vout)
{
  time_limit->set_limit_exceeded_fn(MakeTimeLimitCallback(h5, vout));
}

std::unique_ptr<jhmr::Countdown>
jhmr::fem::MakeCountdown(const std::string& time_format,
                          H5::H5File& h5,
                          std::ostream& vout)
{
...
jhmr::fem::EstCutsAndCreateFragHelper::EstCutsAndCreateFragHelper(const std::string& est_cuts_exe_path,
...
std::string jhmr::fem::EstCutsAndCreateFragHelper::verbose_arg_str() const
{
  return "-v";
  //return "";
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_full_pelvis_seg(const LabelVol* seg)
{
  WriteITKImageToDisk(seg, tmp_full_pelvis_seg_path_);
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_hemipelvis_scalar_seg(const Vol* hemi)
{
  WriteITKImageToDisk(hemi, tmp_hemi_pelvis_seg_path_);
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_invalid_frag_mask(const LabelVol* mask)
{
  WriteITKImageToDisk(mask, tmp_invalid_cuts_path_);
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_anat_lands(const LandMap3& lands)
{
  WriteFCSVFileFromNamePtMap(tmp_lands_path_, lands.begin(), lands.end());
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_regi(const FrameTransform& regi_xform)
{
  WriteITKAffineTransform(tmp_regi_pose_path_, regi_xform);
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_orig_cuts(const PAOCuts& cuts_def, const PAODispInfo& cuts_disp)
{
  WritePAOCutPlanesXML(cuts_def, tmp_init_cuts_xml_path_, &cuts_disp);
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_est_cuts(const PAOCuts& cuts_def, const PAODispInfo& cuts_disp)
{
  WritePAOCutPlanesXML(cuts_def, tmp_final_cuts_xml_path_, &cuts_disp);
}

void jhmr::fem::EstCutsAndCreateFragHelper::set_proj_labels(const std::vector<CutLabelProjData>& proj_labels)
{
  WriteProjData(tmp_label_projs_path_, proj_labels);
}

std::tuple<jhmr::LabelVolPtr,double>
jhmr::fem::EstCutsAndCreateFragHelper::create_frag_seg() const
{
...
std::tuple<jhmr::fem::PAOCuts,jhmr::fem::PAODispInfo,jhmr::PtList3,jhmr::PtList3,double>
jhmr::fem::EstCutsAndCreateFragHelper::est_cuts() const
{
  jhmrASSERT((side_str == "left") || (side_str == "right"));
...
double jhmr::fem::EstCutsAndCreateFragHelper::get_time_from_log() const
{
  double time_secs = 0;
...
jhmr::FrameTransform
jhmr::fem::CorrPtCloudRegi(const LandMap3& dst_lands_map,
...
jhmr::LandMap3 jhmr::fem::ExtractAllIliumPts(const LandMap3& src)
{
  return ExtractPtsPrefixSuffix(src, "IL-", "");
}

jhmr::LandMap3 jhmr::fem::ExtractRightIliumPts(const LandMap3& src)
{
  return ExtractPtsPrefixSuffix(src, "IL-", "-R");
}

jhmr::LandMap3 jhmr::fem::ExtractLeftIliumPts(const LandMap3& src)
{
  return ExtractPtsPrefixSuffix(src, "IL-", "-L");
}

jhmr::LandMap3 jhmr::fem::ExtractIliumPtsSided(const LandMap3& src, const bool is_left)
{
  return is_left ? ExtractLeftIliumPts(src) : ExtractRightIliumPts(src);
}

jhmr::LandMap3 jhmr::fem::ExtractAllFragPts(const LandMap3& src)
{
  return ExtractPtsPrefixSuffix(src, "FR-", "");
}

jhmr::LandMap3 jhmr::fem::ExtractRightFragPts(const LandMap3& src)
{
  return ExtractPtsPrefixSuffix(src, "FR-", "-R");
}

jhmr::LandMap3 jhmr::fem::ExtractLeftFragPts(const LandMap3& src)
{
  return ExtractPtsPrefixSuffix(src, "FR-", "-L");
}

jhmr::LandMap3 jhmr::fem::ExtractFragPtsSided(const LandMap3& src, const bool is_left)
{
  return is_left ? ExtractLeftFragPts(src) : ExtractRightFragPts(src);
}

jhmr::LandMap3 jhmr::fem::ExtractPtsSided(const LandMap3& src, const bool is_left)
{
  return ExtractPtsPrefixSuffix(src, "", is_left ? "-L" : "-R");
}

std::unordered_map<std::string,Eigen::Matrix<double,3,1>> jhmr::fem::LandMapDouble(const LandMap3& f)
{
...
std::unordered_map<std::string,Eigen::Matrix<double,2,1>> jhmr::fem::LandMapDouble(const LandMap2& f)
{
...
jhmr::LandMap2 jhmr::fem::LandMap3To2(const LandMap3& src)
{
  LandMap2 dst;
  dst.reserve(src.size());
...
jhmr::LandMap3 jhmr::fem::ExtractSmallBBs(const LandMap3& src)
{
  const std::vector<char> small_inds = { '1', '2', '3', '4' };
  return ExtractPtsValidInds(src, small_inds);
}

jhmr::LandMap3 jhmr::fem::ExtractLargeBBs(const LandMap3& src)
{
  const std::vector<char> large_inds = { '5', '6', '7', '8' };
  return ExtractPtsValidInds(src, large_inds);
}

std::vector<jhmr::fem::ProjData>
jhmr::fem::GetCamAndBBLands(const H5::Group& g, std::ostream& vout,
                             const bool include_anat_lands,
                             const bool include_pixels)
{
  const H5::Group bb_g = g.openGroup("bb-lands");

  vout << "reading cams..." << std::endl;
  
  std::vector<ProjData> pd(3);
  ...
  for (size_type i = 0; i < 3; ++i)
  {
    ...
    if (include_pixels)
    {
      pd[i] = ReadProjDataH5<PixelScalar>(proj_g)[0];
    }
    else
    {
      pd[i] = std::get<0>(ReadProjInfoH5<PixelScalar>(proj_g))[0];
    }
    ...
    vout << "     bbs..." << std::endl;
    auto bb_lands_fcsv = ReadLandmarksMapH5<Pt3>(bb_g.openGroup(proj_str));
...
std::vector<jhmr::fem::ProjData>
jhmr::fem::GetCamAndBBLands(const H5::Group& g, const H5::Group& pelvis_as_fid_parent_g,
...
jhmr::LandMap3 jhmr::fem::TrianPts(const std::vector<ProjData>& pd)
{
...
jhmr::PtList3 jhmr::fem::SortAPPPtsByAP(const PtList3& src_pts_wrt_app, const size_type axis_idx)
{
...
jhmr::Pt3 jhmr::fem::FindAcetRimPt(const PtList3& rim_pts, const Pt3& fhc_wrt_app, const bool is_left)
{
...
jhmr::CoordScalar
jhmr::fem::LateralCenterEdgeAngle(const Pt3& fhc_wrt_app, const Pt3& acet_rim_pt_wrt_app, const bool is_left)
{
...
jhmr::fem::LateralCenterEdgeAngleAfterRepo(const FrameTransform& frag_xform,
...
std::unordered_map<std::string,boost::any>& jhmr::fem::GlobalDebugDataManager()
{
...
jhmr::LandMap3 jhmr::fem::TransformLandMap(const LandMap3& src, const FrameTransform& xform)
{
  LandMap3 dst;
  dst.reserve(src.size());
...
jhmr::PtList2 jhmr::fem::DetectBBsIn2D(itk::Image<PixelScalar,2>* proj, const bool log_corrected,
                                        const bool find_large_bbs)
{
  ...
  cv::Mat bb_locs_mat = radialSymmetry(log_corrected ? tmp_ocv : proj_ocv,
                                       find_large_bbs ? std::vector<int>({ 4 }) : std::vector<int>({ 1,2 }),
                                       0.2);
...
bool jhmr::fem::GetUseLargeBBs(const H5::CommonFG& h5)
{
