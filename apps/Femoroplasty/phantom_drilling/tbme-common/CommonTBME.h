...
#ifndef COMMONTBME_H_
#define COMMONTBME_H_
...
#include "jhmrCameraRayCastingDepthOnlyGPU.h"
...
PtList2 DetectBBsIn2D(itk::Image<PixelScalar,2>* proj, const bool log_corrected = false,
                      const bool find_large_bbs = false, const bool refine_w_max = false);

bool GetUseLargeBBs(const H5::CommonFG& h5);

VolPtr ConvertLabelVolToFloat(const LabelVol* lv);

struct GTSeg2DHelper
{
  // Inputs:
  
  std::vector<VolPtr> label_vols_as_float;

  std::unordered_map<std::string,size_type> vol_inds;

  // These are labels given in the 2D projection
  LabelScalar bg_label                = 0;
  LabelScalar left_hemi_pelvis_label  = 1;
  LabelScalar right_hemi_pelvis_label = 2;
  LabelScalar vertebrae_label         = 3;
  LabelScalar upper_sacrum_label      = 4;
  LabelScalar lower_sacrum_label      = 7;
  LabelScalar left_femur_label        = 5;
  LabelScalar right_femur_label       = 6;
  LabelScalar soft_tissue_label       = 8;
  LabelScalar left_frag_label         = 9;
  LabelScalar right_frag_label        = 10;

  // Internal state, etc.
  std::unique_ptr<CameraRayCasterDepthOnlyGPU> depth_rc;

  // Output:
  //using LabelImg2D = CutLabelProjData::ImageType;
  using LabelImg2DPtr = CutLabelProjData::ImagePointer;
  LabelImg2DPtr seg;
 
  // channels of the multi-class segmentation

  cv::Mat left_hemi_pelvis_seg;
  cv::Mat right_hemi_pelvis_seg;
  cv::Mat vertebrae_seg;
  cv::Mat upper_sacrum_seg;
  cv::Mat lower_sacrum_seg;
  cv::Mat left_femur_seg;
  cv::Mat right_femur_seg;
  cv::Mat left_frag_seg;
  cv::Mat right_frag_seg;
  cv::Mat soft_tissue_seg;
  cv::Mat bg_seg;
  
  // stores the number of intersections at each pixel
  cv::Mat num_inters_img;
  
  void init(GPUPrefsXML& gpu_prefs, const CamModel& cam);

  void run(const std::unordered_map<std::string,FrameTransform>& vol_poses);
};

CamModel ReadCommonCamModel(const H5::CommonFG& h5);
