...
int main(int argc, char* argv[])
{
  ...
  const std::string device_vol_path = "/Users/gaocong/Documents/Research/Femoroplasty/Phantom_Drilling/meta_data/Device_crop_CT.nii.gz";
  const std::string device_seg_path = "/Users/gaocong/Documents/Research/Femoroplasty/Phantom_Drilling/meta_data/Device_crop_seg.nii.gz";
  ...
  const std::string femur_xform_file_path = init_xform_folder + "/femur_regi_xform" + img_ID_list[0] + ".h5";
                         ...
                         &init_device_xform, &init_femur_xform, &output_path, &kSAVE_REGI_DEBUG] (const size_type view_idx)
  {
    ...
    regi.debug_info.regi_names = { { "Device" + fmt::sprintf("%03lu", view_idx) + "-Coarse" } };
...

    {
      ...
      lvl_regi_coarse.ref_frames = { 0 };
      {
        ...
        pen_fn->trans_x_pdf = NormPDF(0, 30);
        pen_fn->trans_y_pdf = NormPDF(0, 30);
        pen_fn->trans_z_pdf = NormPDF(0, 150);
      ...
      vout << std::endl << "View " << fmt::sprintf("%03lu", view_idx) << " device coarse registration ..." << std::endl;
      regi.run();
      ...
      if(kSAVE_REGI_DEBUG){
        ...
        WriteRegi2D3DMultiLevelDebug(regi.debug_info, output_path+"/device_coarse_debug" + fmt::sprintf("%03lu", view_idx) + ".h5",
                                    std::make_tuple(VolPtr(), LabelVolPtr(), tmp_pd_list));
      }

      // First View Spine Fine Registration
      {
        auto& lvl = regi.levels[0];
        lvl.ds_factor = 0.25;
        ...
        regi.debug_info.regi_names = { { "Device" + fmt::sprintf("%03lu", view_idx) + "-Fine" } };
        regi.init_cam_to_vols = { regi.cur_cam_to_vols[0],  init_femur_xform};

        lvl.regis.resize(1);
        {
          auto& lvl_regi_fine = lvl.regis[0];
          lvl_regi_fine.mov_vols = { 0 };
          lvl_regi_fine.static_vols = { };
          ...
          lvl_regi_fine.init_mov_vol_poses = { device_init_guess };
          lvl_regi_fine.ref_frames = { 0 };
          auto bobyqa_regi = std::make_shared<BOBYQARegi>();
          {
            ...
            //bobyqa_regi->set_bounds({ 20 * kDEG2RAD, 20 * kDEG2RAD, 15 * kDEG2RAD,
            //                          10, 10, 100 });
            bobyqa_regi->set_bounds({ 2.5 * kDEG2RAD, 2.5 * kDEG2RAD, 2.5 * kDEG2RAD,
                                      5, 5, 10 });
            
            lvl_regi_fine.regi = bobyqa_regi;
          }
        }
        vout << std::endl << "View " << fmt::sprintf("%03lu", view_idx) << " device fine registration ..." << std::endl;
        regi.run();
        regi.levels[0].regis[0].fns_to_call_right_before_regi_run.clear();
        ...
        if(kSAVE_REGI_DEBUG){
          ...
          WriteRegi2D3DMultiLevelDebug(regi.debug_info, output_path+"/device_fine_debug" + fmt::sprintf("%03lu", view_idx) + ".h5",
                                      std::make_tuple(VolPtr(), LabelVolPtr(), tmp_pd_list));
        }
      }
  ...
  // Femur Registration-Coarse
  {
    ...
    lvl.regis.resize(1);
    {
      ...
      cmaes_regi->set_sigma({ 1 * kDEG2RAD, 1 * kDEG2RAD, 1 * kDEG2RAD, 5, 5, 5});
    
      auto pen_fn = std::make_shared<PenFn>();
      pen_fn->rot_pdfs_per_obj.assign(1, FoldNormPDF(3 * kDEG2RAD, 3 * kDEG2RAD));
      pen_fn->trans_pdfs_per_obj.assign(1, FoldNormPDF(7.5, 7.5));
  ...
  
  {
    ...
    {
      const std::string src_ureef_path          = "/Users/gaocong/Documents/Research/Femoroplasty/Phantom_Drilling/Registration/UR_kins/31/ur_eef.h5";
    ...
    {
      const std::string src_ureef_path          = "/Users/gaocong/Documents/Research/Femoroplasty/Phantom_Drilling/Registration/UR_kins/32/ur_eef.h5";
      H5::H5File h5_ureef(src_ureef_path, H5F_ACC_RDWR);
      H5::Group ureef_transform_group           = h5_ureef.openGroup("TransformGroup");
      H5::Group ureef_group0                    = ureef_transform_group.openGroup("0");
      std::vector<float> UReef_tracker          = ReadVectorH5<float>("TranformParameters", ureef_group0);
    ...
    FrameTransform reproj_device_xform = drill_rotcen_ref.inverse() * handeye_regi_X.inverse() * repos_UReef_xform.inverse() * regi_UReef_xform * handeye_regi_X * drill_rotcen_ref * ref_device_xform;
    ...
    auto repos_real_ptr = ReadITKImageFromDisk<Proj>("/Users/gaocong/Documents/Research/Femoroplasty/Phantom_Drilling/Registration/RealXray_dcm/40");
