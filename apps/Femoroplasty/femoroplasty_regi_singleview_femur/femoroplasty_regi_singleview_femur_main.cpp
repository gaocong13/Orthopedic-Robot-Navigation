...
int main(int argc, char* argv[])
{
  ...
  const std::string femur_3d_fcsv_path = meta_data_path + "/femur_anatomical_landmark.fcsv";
  ...
  std::shared_ptr<MultiLevelMultiObjRegi::CamAlignRefFrameWithCurPose> femur_singleview_regi_ref_frame;
  {
    ...
    auto femur_fcsv_rotc = femur_3d_fcsv.find("FH-r");
    Pt3 rotcenter;
  ...
  for(int idx=0; idx<num_views; ++idx)
  {
    ...
    const std::string femur_landmark_2d_fcsv_path   = landmark2d_root_path + "/femur" + exp_ID + ".fcsv";
    const std::string img_path                      = dicom_path + "/" + exp_ID;

    std::cout << "Running..." << exp_ID << std::endl;
    auto femur_landmark_2d_fcsv = ReadFCSVFileNamePtMap(femur_landmark_2d_fcsv_path);
    ConvertRASToLPS(&femur_landmark_2d_fcsv);

    xregASSERT(femur_landmark_2d_fcsv.size() > 3);
    ...
    {
      UpdateLandmarkMapForCIOSFusion(cios_metas[0], &femur_landmark_2d_fcsv);

      auto& pelvis_proj_lands = proj_pre_proc.input_projs[0].landmarks;
      pelvis_proj_lands.reserve(femur_landmark_2d_fcsv.size());

      for (const auto& fcsv_kv : femur_landmark_2d_fcsv)
      {
    ...
    // Femur Registration Next
    {
...

      {
        ...
        lvl_regi_fine.ref_frames = { 0 }; // This refers to pelvis (moving)
        {
          auto cmaes_regi = std::make_shared<Intensity2D3DRegiCMAES>();
