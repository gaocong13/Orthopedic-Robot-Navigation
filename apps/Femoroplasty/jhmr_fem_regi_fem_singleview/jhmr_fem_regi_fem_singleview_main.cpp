...
int main(int argc, char* argv[])
{
  ...
  std::shared_ptr<MultiLevelMultiObjRegi::StaticRefFrame> femur_multiview_regi_ref_frame;
  {
    ...
    // setup camera aligned reference frame, use femur volume center point as the origin
  
    const auto vol_size = femur_vol->GetLargestPossibleRegion().GetSize();
    ...
    femur_vol->TransformContinuousIndexToPhysicalPoint(center_idx, center_pt);
    ...
    femur_singleview_regi_ref_frame->center_of_rot_wrt_vol[0] = center_pt[0];
    femur_singleview_regi_ref_frame->center_of_rot_wrt_vol[1] = center_pt[1];
    femur_singleview_regi_ref_frame->center_of_rot_wrt_vol[2] = center_pt[2];
    
    FrameTransform femur_vol_to_centered_vol = FrameTransform::Identity();
    femur_vol_to_centered_vol.matrix()(0,3) = -center_pt[0];
    femur_vol_to_centered_vol.matrix()(1,3) = -center_pt[1];
    femur_vol_to_centered_vol.matrix()(2,3) = -center_pt[2];
