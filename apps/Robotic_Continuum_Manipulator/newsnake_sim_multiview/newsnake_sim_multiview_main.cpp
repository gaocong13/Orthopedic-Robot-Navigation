...
int main(int argc, char* argv[])
{
  ...
  ProjPreProc proj_bone_preproc, proj_snake_preproc;
  proj_bone_preproc.set_debug_output_stream(vout, verbose);
  proj_snake_preproc.set_debug_output_stream(vout, verbose);
  proj_bone_preproc.input_projs.resize(num_views);
  proj_snake_preproc.input_projs.resize(num_views);

  // ********** TMRB C-arm Projection Geometry ***************************************************************
  const auto default_cam = NaiveCamModelFromCIOSFusion(MakeNaiveCIOSFusionMetaDR(), true);

  CamModelList cams, cams_snake;
  cams.reserve(num_views);
  cams_snake.reserve(num_views);

  FrameTransform gt_cam_wrt_app = CreateAPViewOfAPP(default_cam, Float(0.8), true, is_left);
  FrameTransform gt_cam_wrt_fem;
  // Perturb multi-view C-arm geometry
  {
    proj_snake_preproc.input_projs[0].cam = default_cam;

    std::array<CoordScalar,2> mean_rot_angs       = { -15, 20};
    std::array<CoordScalar,2> mean_snake_rot_angs = { -30, 30};
    std::array<CoordScalar,2> std_dev_rot_angs    = { 3, 3};

    UniformOnUnitSphereDist unit_vec_rng(3);

    for (size_type cam_idx = 1; cam_idx < num_views; ++cam_idx)
    {
      const auto& ref_cam = proj_snake_preproc.input_projs[0].cam;
      ...
      const CoordScalar cur_snake_rot_ang_rad = std::normal_distribution<CoordScalar>(
          mean_snake_rot_angs[cam_idx-1], std_dev_rot_angs[cam_idx-1])(rng_eng) * kDEG2RAD;

      FrameTransform carm_rand_perturb = FrameTransform::Identity();
      {
        Pt3 so3 = unit_vec_rng(rng_eng);
        so3 *= carm_small_rot_ang_dist(rng_eng);

        auto rot = ExpPtToRot3x3(so3);

        Pt3 trans = unit_vec_rng(rng_eng);
        trans *= carm_small_trans_mag_dist(rng_eng);

        carm_rand_perturb.matrix().block(0,0,3,3) = rot;
        carm_rand_perturb.matrix().block(0,3,3,1) = trans;
      }

      const FrameTransform new_extrins = ref_cam.extrins * EulerRotXFrame(cur_rot_ang_rad) * carm_rand_perturb;
      {
        auto& cur_cam = proj_snake_preproc.input_projs[cam_idx].cam;
    ...
    for (const auto& pd : proj_snake_preproc.input_projs)
    {
      cams_snake.push_back(pd.cam);
    }

    std::uniform_real_distribution<CoordScalar> rot_ang_dist(-3*kDEG2RAD, 3*kDEG2RAD);
    std::uniform_real_distribution<CoordScalar> trans_mag_dist(0,10);

    CoordScalar pelvis_app_rot_angZ = -10 * kDEG2RAD;
    FrameTransform pelvis_rot_matZ = EulerRotZFrame(pelvis_app_rot_angZ);

    CoordScalar pelvis_app_rot_angY = -15 * kDEG2RAD;
    FrameTransform pelvis_rot_matY = EulerRotYFrame(pelvis_app_rot_angY);

    CoordScalar pelvis_app_rot_angX = -20 * kDEG2RAD;
    FrameTransform pelvis_rot_matX = EulerRotXFrame(pelvis_app_rot_angX);
    ...
    CoordScalar cam_rot_angZ = -20 * kDEG2RAD;
    FrameTransform cam_rot_matZ = EulerRotZFrame(cam_rot_angZ);

    CoordScalar cam_rot_angY = 20 * kDEG2RAD;
    FrameTransform cam_rot_matY = EulerRotZFrame(cam_rot_angY);

    CoordScalar cam_rot_angX = 20 * kDEG2RAD;
    FrameTransform cam_rot_matX = EulerRotXFrame(cam_rot_angX);

    FrameTransform cam_rot_mat = cam_rot_matX;

    delta_cam.matrix().block(0,3,3,1) = cam_trans;
    //delta_cam.matrix().block(0,0,3,3) = cam_rot_mat.matrix().block(0,0,3,3);
    ...
    auto rot_mat = ExpPtToRot3x3(so3);

    FrameTransform delta_app = FrameTransform::Identity();

    delta_app.matrix().block(0,0,3,3) = rot_mat;
    ...
    gt_cam_wrt_fem = app_to_ct * delta_app * gt_cam_wrt_app;
  }
  ...
  for (size_type exp_idx = 0; exp_idx < num_exp; ++exp_idx)
  {
...

    {
      ...
      ray_caster->set_camera_models(cams_snake);
      ...
      for (size_type proj_idx = 0; proj_idx < num_views; ++proj_idx)
      {
        proj_snake_preproc.input_projs[proj_idx].img = CastITKImageIfNeeded<float>(SamplePoissonProjFromAttProj(ray_caster->proj(proj_idx).GetPointer(), 5000).GetPointer());
        WriteITKImageRemap8bpp(proj_snake_preproc.input_projs[proj_idx].img.GetPointer(), output_path + fmt::sprintf("/snake_proj_img%03lu_exp%03lu_view%03lu", model_idx, exp_idx, proj_idx) + ".png");
      }
    }
    ...
    {
      Pt3 ld1 = proj_snake_preproc.input_projs[view_idx].cam.phys_pt_to_ind_pt(Pt3(gt_allseg_xforms[0].inverse() *  lands_3d[0]));
      Pt3 ld2 = proj_snake_preproc.input_projs[view_idx].cam.phys_pt_to_ind_pt(Pt3(gt_allseg_xforms[num_snake_vols-1].inverse() * lands_3d[1]));
      ...
      {
        auto& proj_lands = proj_snake_preproc.input_projs[view_idx].landmarks;
    ...
    proj_snake_preproc();
    auto& proj_data_snake = proj_snake_preproc.output_projs;

    if(kSAVE_REGI_DEBUG)
    {
      const std::string proj_data_snake_h5_path = output_path + "/snake_multiview_proj_data" + exp_ID + ".h5";
      vout << "creating H5 proj bone data file for exp" + exp_ID + "..." << std::endl;
      H5::H5File h5(proj_data_snake_h5_path, H5F_ACC_TRUNC);
      WriteProjDataH5(proj_data_snake, &h5);

      for(size_type view_idx = 0; view_idx < num_views; ++view_idx)
      {
        WriteITKImageRemap8bpp(proj_data_snake[view_idx].img.GetPointer(), output_path + fmt::sprintf("/snake_regi_img%03lu_exp%03lu_view%03lu", model_idx, exp_idx, view_idx) + ".png");
      }
    }
    ...
    regi_snake.fixed_proj_data = proj_data_snake;
    ...
    if (kSAVE_REGI_DEBUG)
    {
      ...
      regi_snake.debug_info->regi_names = { { "Multiview Snake" + exp_ID } };
    }
