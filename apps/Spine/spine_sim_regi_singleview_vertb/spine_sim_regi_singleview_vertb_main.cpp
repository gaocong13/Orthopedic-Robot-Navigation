...
int main(int argc, char* argv[])
{
  ...
  // auto vert1_hu = ApplyMaskToITKImage(spinevol_hu.GetPointer(), spine_seg.GetPointer(), vert1_label, float(0), true);
  // WriteITKImageToDisk(vert1_att.GetPointer(), meta_data_path + "/vert1_att.nii.gz");
  // WriteITKImageToDisk(vert1_hu.GetPointer(), meta_data_path + "/vert1_hu.nii.gz");

  auto vert2_att = ApplyMaskToITKImage(spine_att.GetPointer(), spine_seg.GetPointer(), vert2_label, float(0), true);
  // auto vert2_hu = ApplyMaskToITKImage(spinevol_hu.GetPointer(), spine_seg.GetPointer(), vert2_label, float(0), true);
  // WriteITKImageToDisk(vert2_att.GetPointer(), meta_data_path + "/vert2_att.nii.gz");
  // WriteITKImageToDisk(vert2_hu.GetPointer(), meta_data_path + "/vert2_hu.nii.gz");

  auto vert3_att = ApplyMaskToITKImage(spine_att.GetPointer(), spine_seg.GetPointer(), vert3_label, float(0), true);
  // auto vert3_hu = ApplyMaskToITKImage(spinevol_hu.GetPointer(), spine_seg.GetPointer(), vert3_label, float(0), true);
  // WriteITKImageToDisk(vert3_att.GetPointer(), meta_data_path + "/vert3_att.nii.gz");
  // WriteITKImageToDisk(vert3_hu.GetPointer(), meta_data_path + "/vert3_hu.nii.gz");

  auto vert4_att = ApplyMaskToITKImage(spine_att.GetPointer(), spine_seg.GetPointer(), vert4_label, float(0), true);
  // auto vert4_hu = ApplyMaskToITKImage(spinevol_hu.GetPointer(), spine_seg.GetPointer(), vert4_label, float(0), true);
  // WriteITKImageToDisk(vert4_att.GetPointer(), meta_data_path + "/vert4_att.nii.gz");
  // WriteITKImageToDisk(vert4_hu.GetPointer(), meta_data_path + "/vert4_hu.nii.gz");

  auto sacrum_att = ApplyMaskToITKImage(spine_att.GetPointer(), sacrum_seg.GetPointer(), sacrum_label, float(0), true);
  // auto sacrum_hu = ApplyMaskToITKImage(spinevol_hu.GetPointer(), sacrum_seg.GetPointer(), sacrum_label, float(0), true);
  // WriteITKImageToDisk(sacrum_att.GetPointer(), meta_data_path + "/sacrum_att.nii.gz");
  // WriteITKImageToDisk(sacrum_hu.GetPointer(), meta_data_path + "/sacrum_hu.nii.gz");
...
/*
  // This piece of program is to validate the computation of vert xforms after repositioning
  {
    ProjDataF32 proj_vertbs;
    auto ray_caster = LineIntRayCasterFromProgOpts(po);
    ray_caster->set_camera_models(cams);
    ray_caster->set_volumes({vert1_att, vert2_att, vert3_att, vert4_att, sacrum_att}); //spine_vol_att
    ray_caster->set_num_projs(1);
    ray_caster->allocate_resources();
    FrameTransformList vert_xforms = { recomp_vertb.GetVert1Xform(),
    ...
    {
      ray_caster->distribute_xform_among_cam_models(vert_refs[idx].inverse() * vert_xforms[idx] * vert_refs[idx] * gt_cam_wrt_spine);
      ray_caster->compute(idx);
      ray_caster->use_proj_store_accum_method();
    }

    proj_vertbs.img = AddPoissonNoiseToImage(ray_caster->proj(0).GetPointer(), 5000);
    WriteITKImageRemap8bpp(proj_vertbs.img.GetPointer(), output_path + "/vertbs.png");
  }
*/
  ...
  for(int idx=0; idx < 1000; ++idx)
  {
    ...
    vout << "Setting up spine rigid registration..." << std::endl;
    ...
    regi_rigid.ref_frames = { spine_singleview_regi_ref_frame };
    ...
    regi_rigid.init_cam_to_vols = { init_cam_wrt_spine };

    spine_singleview_regi_ref_frame->cam_extrins = regi_rigid.fixed_proj_data[view_idx].cam.extrins;
    ...
    vout << std::endl << "Single view spine rigid registration ..." << std::endl;
    regi_rigid.run();
    regi_rigid.levels[0].regis[0].fns_to_call_right_before_regi_run.clear();

    const FrameTransform spine_rigid_regi_xform = regi_rigid.cur_cam_to_vols[0];
    ...
    if (kSAVE_REGI_DEBUG)
    {
      ...
      ray_caster->set_num_projs(1);
      ray_caster->allocate_resources();
      ray_caster->xform_cam_to_itk_phys(0) = regi_rigid.cur_cam_to_vols[0];
      ray_caster->compute(0);
    ...
    regi_vertb.ref_frames = { vert1_singleview_regi_ref_frame, vert2_singleview_regi_ref_frame,
    ...
    vert1_singleview_regi_ref_frame->cam_extrins = regi_vertb.fixed_proj_data[view_idx].cam.extrins;
    vert2_singleview_regi_ref_frame->cam_extrins = regi_vertb.fixed_proj_data[view_idx].cam.extrins;
    vert3_singleview_regi_ref_frame->cam_extrins = regi_vertb.fixed_proj_data[view_idx].cam.extrins;
    vert4_singleview_regi_ref_frame->cam_extrins = regi_vertb.fixed_proj_data[view_idx].cam.extrins;
    sacrum_singleview_regi_ref_frame->cam_extrins = regi_vertb.fixed_proj_data[view_idx].cam.extrins;
    device_singleview_regi_ref_frame->cam_extrins = regi_vertb.fixed_proj_data[view_idx].cam.extrins;
    ...
    vout << std::endl << "Single view vertb registration ..." << std::endl;
    regi_vertb.run();
    ...
    if (kSAVE_REGI_DEBUG)
    {
      ...
      for(size_type idx = 0; idx < 5; ++idx)
      {
        ray_caster->distribute_xform_among_cam_models(regi_vertb.cur_cam_to_vols[idx]);
        ray_caster->compute(idx);
        ray_caster->use_proj_store_accum_method();
      }
    ...
    vout << "Setting up device registration..." << std::endl;
    ...
    regi_device.ref_frames = { device_singleview_regi_ref_frame };

    regi_device.fixed_proj_data = proj_device_list;

    regi_device.levels.resize(1);

    regi_device.init_cam_to_vols = { init_cam_wrt_device };
    // Device Single-view Registration
    {
      ...
      if (kSAVE_REGI_DEBUG )
      {
        ...
        regi_device.debug_info->regi_names = { { "Singleview Device" + exp_ID } };
      }
    }
    ...
    if (kSAVE_REGI_DEBUG)
    {
      ...
      ray_caster->set_num_projs(1);
      ray_caster->allocate_resources();
      ray_caster->xform_cam_to_itk_phys(0) = regi_device.cur_cam_to_vols[0];
      ray_caster->compute(0);
