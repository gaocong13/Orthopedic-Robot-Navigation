...
int main(int argc, char* argv[])
{
  ...
  po.set_arg_usage("< Landmark Annotation Folder Path > < Meta Data Path > < DCM Image Folder Path > < Output Path >");
  po.set_min_num_pos_args(4);
  po.add("start-frame", 's', ProgOpts::kSTORE_INT32, "start-frame",
         "The start frame ID. Default is 0. Corresponding landmark annotation file (fluoro${frame}.csv) should exist in the landmark annotation folder!")
    << 0;
  po.add("num-frames", 'n', ProgOpts::kSTORE_INT32, "num-frames",
         "Total number of fluoro frames as integer. Default is 900")
    << 900;
  ...
  const std::string output_path             = po.pos_args()[3];  // Output path
  ...
  const size_type start_frame = po.get("start-frame").as_int32();
  const size_type total_frames = po.get("num-frames").as_int32();
  ...
  // Read start frame landmark annotation
  const std::string spinebb_2d_fcsv_path  = landmark2d_root_path + "/fluoro" + fmt::format("{:03d}", start_frame) + ".fcsv";
  ...
  FrameTransform init_cam_to_spine;

  const std::string example_dcm_img_path = dicom_path + "/../003";

  for(int idx = start_frame; idx < total_frames; ++idx)
  {
    const std::string exp_ID                = fmt::format("{:03d}", idx);
    const std::string img_path              = dicom_path + "/" + exp_ID + ".tiff";
    ...
    {
      std::tie(proj_pre_proc.input_projs[0].img, spinecios_metas[0]) = ReadCIOSFusionDICOMFloat(example_dcm_img_path);
      proj_pre_proc.input_projs[0].img = ReadITKImageFromDisk<itk::Image<float,2>>(img_path);
      proj_pre_proc.input_projs[0].img->SetSpacing(0.194);
    ...
    if( idx == start_frame )
    {
      init_cam_to_spine = PnPPOSITAndReprojCMAES(projs_to_regi[0].cam, spine_3d_fcsv, projs_to_regi[0].landmarks);
      WriteITKAffineTransform(output_path + "/xform/landmark_init_xform" + exp_ID + ".h5", init_cam_to_spine);
    }
    else
    {
      std::string prev_regi_xform_file = output_path + "/xform/spine_regi_xform" + fmt::format("{:03d}", idx-1) + ".h5";
      init_cam_to_spine = ReadITKAffineTransformFromFile( prev_regi_xform_file );
    }
    ...
    if (kSAVE_REGI_DEBUG )
    {
      ...
      const std::string proj_data_h5_path = output_path + "/projdata/proj_data" + exp_ID + ".h5";
    ...
    if (kSAVE_REGI_DEBUG)
    {
      ...
      const std::string dst_debug_path = output_path + "/debug/debug_spine" + exp_ID + ".h5";
      WriteMultiLevel2D3DRegiDebugToDisk(*regi.debug_info, dst_debug_path);
    }

    WriteITKAffineTransform(output_path + "/xform/spine_regi_xform" + exp_ID + ".h5", regi.cur_cam_to_vols[0]);

    {
      ...
      WriteITKImageRemap8bpp(ray_caster->proj(0).GetPointer(), output_path + "/projdata/spine_reproj" + exp_ID + ".png");
      WriteITKImageRemap8bpp(proj_pre_proc.input_projs[0].img.GetPointer(), output_path + "/projdata/real" + exp_ID + ".png");
    }
  }
  return 0;
}
