...
int main(int argc, char* argv[])
{
  ...
  const std::string handeye_X_path            = po.pos_args()[4];  // Handeye Init path
  ...
  vout << "reading device screw landmarks from FCSV file..." << std::endl;
  auto device_3d_fcsv = ReadFCSVFileNamePtMap(device_3d_fcsv_path);
  ConvertRASToLPS(&device_3d_fcsv);

  vout << "setting up reference frame..." << std::endl;
  ...
  for(size_type view_idx = 0; view_idx < num_views; ++view_idx)
  {
    ...
    const std::string device_2d_fcsv_path = device_2d_fcsv_root_path + "/reproj_screws" + exp_ID_list[view_idx] + ".fcsv";
...

  {
    ...
    {
      auto cmaes_regi = std::make_shared<Intensity2D3DRegiCMAESdeviceMultiConfigOptimHandeye>();
      ...
      cmaes_regi->set_handeyeX_save_path(output_path);
